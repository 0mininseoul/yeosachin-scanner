...
- `POST /v1/profiles` adds `usernames`, an ordered unique array of 1–30 usernames. Its standard envelope has one entry per requested username, in request order: `{ "username": "…", "status": "available", "profile": { … } }`, `{ "username": "…", "status": "not_found" }`, or `{ "username": "…", "status": "failed", "failureCategory": "schema" }`. Only the provider's known user-not-found result becomes `not_found`; a malformed provider object is isolated to its username as the explicit `schema` failure row so no profile evidence is fabricated. Account/authentication, rate-limit, and other unexpected upstream failures retain the normal sanitized batch-level error behavior.
