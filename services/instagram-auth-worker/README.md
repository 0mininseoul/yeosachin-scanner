...
- `IG_DURABLE_STORE_BUCKET`: required in production. Dedicated GCS bucket for the durable operation ledger and account-safety state. If this configuration or its object access is unavailable, production must fail closed; there is no in-memory fallback.
