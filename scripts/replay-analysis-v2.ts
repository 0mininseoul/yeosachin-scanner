...
import { createReplayKeyFile, readReplayBundle, removeReplayArtifacts, writeReplayBundle } from '../lib/services/analysis/replay/replay-bundle';
...
async function capture(options: Extract<ReplayCliOptions, { command: 'capture' }>): Promise<void> {
    ...
    process.stdout.write(`${JSON.stringify({ status: 'ok', command: 'capture', profiles: bundle.profiles.length, public: bundle.profiles.filter(p => !p.isPrivate).length, private: bundle.profiles.filter(p => p.isPrivate).length, media: bundle.profiles.reduce((sum, p) => sum + p.media.length, 0), bytes, elapsed_ms: Math.round(performance.now() - started) })}\n`);
}
