...
function client(payload: unknown, error: unknown = null) {
    const maybeSingle = vi.fn().mockResolvedValue({ data: payload === null ? null : { version: 'operator-editable-fixture-v1', status: 'draft', payload }, error });
...
describe('controlled demo fixture publisher', () => {
    it('does not promote a Zod-invalid draft', async () => {
        ...
        await expect(publishDemoEditableFixture(db as never, 'operator-editable-fixture-v1')).rejects.toThrow(/invalid/i);
    ...
    it('passes the exact validated payload to the atomic database publisher', async () => {
        ...
        await publishDemoEditableFixture(db as never, 'operator-editable-fixture-v1');
        expect(db.rpc).toHaveBeenCalledWith('publish_demo_analysis_fixture', {
            p_version: 'operator-editable-fixture-v1', p_expected_payload: payload,
        });
    });
});
