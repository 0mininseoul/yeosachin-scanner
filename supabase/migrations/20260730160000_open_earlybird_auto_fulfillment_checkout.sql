-- Automatic fulfillment is now the normal path.  New checkout records a
-- time-free disclosure, while an exact legacy disclosure remains accepted only
-- to finish/replay immutable orders created by the previous rollout.
--
-- This intentionally does not update, cancel, supersede, or otherwise mutate
-- existing orders. In particular, only unresolved payment_pending or
-- no-payment cancelled lineages block a fresh checkout; a paid
-- terminal/manual-review fulfillment remains an immutable historical order and
-- cannot block a distinct fresh preflight.
...
BEGIN
    ...
    -- Exact retry is the only replay that returns a checkout. It preserves the
    -- immutable payment snapshot even while the app disclosure rolls over.
    SELECT existing_order.*
    INTO v_existing
    FROM public.earlybird_orders AS existing_order
    WHERE existing_order.preflight_id = p_preflight_id;
    IF v_legacy_disclosure AND (
        NOT FOUND
        OR v_existing.disclosure_version IS DISTINCT FROM 'earlybird-24h-v1'
        OR v_existing.disclosure_text IS DISTINCT FROM '현재 얼리버드 기간에는 즉시 자동 판독이 아닌, 결제 완료 후 24시간 이내 판독 결과를 제공합니다.'
    ) THEN
        RAISE EXCEPTION 'EARLYBIRD_CONSENT_INVALID';
    END IF;
    IF FOUND THEN
        IF v_existing.user_id <> p_user_id
           ...
           OR v_existing.expected_buyer_phone_verification_source
                IS DISTINCT FROM v_user_phone_number_verification_source THEN
        ...
        IF v_existing.status = 'cancelled' AND v_existing.payment_id IS NULL THEN
            IF v_existing.pricing_version = 'earlybird-2026-07-v1' THEN
    ...
    END IF;
    IF v_preflight.pricing_version = 'earlybird-2026-07-v1'
       AND p_pricing_version = 'earlybird-2026-07-v2' THEN
    ...
    -- A late payment can still be attributed to this same-product cancelled
    -- snapshot. Do not replace it with another order.
    SELECT unresolved_order.*
    INTO v_existing
    ...
    ORDER BY unresolved_order.created_at, unresolved_order.id
    LIMIT 1;
    IF FOUND THEN
        IF v_existing.pricing_version = 'earlybird-2026-07-v1' THEN
            RAISE EXCEPTION 'EARLYBIRD_CHECKOUT_CANCELLED_UNRESOLVED_LINEAGE:STALE_PRICING_LINEAGE';
    ...
    -- Do not cancel, supersede, expire, or replace a live checkout. A prior
    -- paid/terminal order is deliberately absent from this predicate so a
    -- later fresh preflight remains eligible for a distinct checkout.
