-- Re-attribute a late payment for a cancelled order when Groble applied a
-- discount. The actual amount must remain non-negative and no greater than the
-- checkout snapshot; all other matching, locking, and inventory rules are kept.
