-- Legacy request rows can retain a presentation-form Instagram handle even
-- though the paid order stores the canonical handle. Recovery accepts only a
-- bounded canonical equivalence; every payment, lifecycle, and snapshot fence
-- remains unchanged.
...
BEGIN
    ...
    IF v_failed_request_target IS DISTINCT FROM v_order_target THEN
        RAISE EXCEPTION USING
            MESSAGE = 'EARLYBIRD_SCHEMA_FAILURE_RECOVERY_INELIGIBLE', ERRCODE = 'P0001';
    END IF;
