-- Keep existing v1 demonstrations readable as historical rows while v2 uses
-- its own fixture namespace and server-owned shorter duration.
...
ALTER TABLE public.demo_analysis_runs
    ADD CONSTRAINT demo_analysis_runs_fixture_version_duration_check CHECK (
        ...
        OR (fixture_version = 'authorized-text-fixture-v2' AND duration_seconds BETWEEN 30 AND 45)
    );
...
BEGIN
    IF p_target_instagram_id !~ '^[A-Za-z0-9._]{1,30}$'
        ...
        RAISE EXCEPTION 'invalid demo v2 run input';
    END IF;
    ...
    WHERE demo_run.user_id = p_user_id
      ...
      AND demo_run.fixture_version = 'authorized-text-fixture-v2';
END;
$$;
