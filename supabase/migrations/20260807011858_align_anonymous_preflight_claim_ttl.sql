-- The browser claim must cover the complete anonymous preflight lifetime. The
-- previous ten-minute limit was shorter than the 30-minute Cloud Tasks retry
-- window, so a delayed but successful preflight became unreadable to its owner.
...
BEGIN
    IF p_target_instagram_id IS NULL
       ...
       OR p_claim_expires_at > v_now + INTERVAL '30 minutes'
    ...
    -- Serialize the replay/create decision for one anonymous idempotency key.
    -- The partial unique index remains the database invariant.
