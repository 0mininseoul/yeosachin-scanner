-- Failed requests remain retained for operations but are not part of the owner-facing history.
...
COMMENT ON FUNCTION public.load_analysis_owner_history_v1() IS
    'Authenticated owner-only history projection. Failed requests remain retained but are excluded; completed V2 usernames come from the final summary.';
