-- The request's retained token is the authoritative scrub lineage. Older
-- consumed preflights can retain a different, but still safe, retained token.
-- Mirror the immutable successor lineage in the existing admission ledger so
-- the request-generation guard can create a fresh idempotency key exactly once.

CREATE OR REPLACE FUNCTION public.recover_earlybird_profile_evidence_failed_fulfillment(
    p_order_id UUID,
...
DECLARE
    ...
    v_schema_recovery public.earlybird_schema_failure_recoveries%ROWTYPE;
...
BEGIN
    ...
    IF FOUND THEN
        ...
        IF NOT FOUND
           ...
           OR (
                v_fulfillment.status NOT IN ('admission_pending', 'retryable_failure')
                AND NOT (
                    v_fulfillment.status = 'manual_review'
                    AND v_fulfillment.last_error_code = 'REQUEST_CONFLICT'
                )
           ) THEN
            RAISE EXCEPTION USING
                MESSAGE = 'EARLYBIRD_PROFILE_EVIDENCE_RECOVERY_CONFLICT',
        ...
        INSERT INTO public.earlybird_schema_failure_recoveries(
            ...
            v_order.id, v_existing.failed_request_id,
            v_existing.recovery_preflight_id, v_existing.prior_attempt_count
        ) ON CONFLICT (order_id) DO NOTHING;
        SELECT schema_recovery.*
        INTO v_schema_recovery
        FROM public.earlybird_schema_failure_recoveries AS schema_recovery
        WHERE schema_recovery.order_id = v_order.id;
        IF NOT FOUND
           OR v_schema_recovery.failed_request_id
                IS DISTINCT FROM v_existing.failed_request_id
           OR v_schema_recovery.recovery_preflight_id
                IS DISTINCT FROM v_existing.recovery_preflight_id THEN
            RAISE EXCEPTION USING
                MESSAGE = 'EARLYBIRD_PROFILE_EVIDENCE_RECOVERY_CONFLICT',
                ERRCODE = 'P0001';
        END IF;
