-- MIGRATION_PREDECESSOR=20260815170000
-- Forward-only, immutable full-copy replacement for the already published
-- first paid concierge result. It is CAS-bound to the exact v2.12 audit
-- payload and can only update the public overview/narrative copy fields.
BEGIN;
...
BEGIN
    IF pg_catalog.to_regclass('supabase_migrations.schema_migrations') IS NOT NULL
       AND NOT EXISTS (
           ...
           WHERE version = '20260815170000'
       ) THEN
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_PREDECESSOR_MISSING', ERRCODE = 'P0001';
    END IF;
END;
$migration$;
...
REVOKE ALL ON TABLE public.earlybird_v213_concierge_copy_corrections
...
CREATE FUNCTION public.correct_earlybird_v213_concierge_copy(
    p_order_id UUID,
    p_owner_id UUID,
...
DECLARE
    ...
    v_prior public.earlybird_v212_concierge_copy_corrections%ROWTYPE;
    v_existing public.earlybird_v213_concierge_copy_corrections%ROWTYPE;
...
BEGIN
    IF p_order_id IS NULL OR p_owner_id IS NULL OR p_result_request_id IS NULL
       ...
       OR p_copy_payload->>'qualityVersion' IS DISTINCT FROM 'v213-full-evidence-review-v1'
       ...
       OR pg_catalog.jsonb_array_length(p_copy_payload->'rows') IS DISTINCT FROM 16 THEN
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_INPUT_INVALID', ERRCODE = 'P0001';
    END IF;

    -- Preserve the exact historical one-shot boundary; this correction does
    -- not rearm admission, billing, fulfillment, or provider execution.
    ...
    IF v_candidate_count <> 1 THEN
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_ORDER_SCOPE_CONFLICT', ERRCODE = 'P0001';
    END IF;
    ...
    SELECT correction.* INTO v_prior FROM public.earlybird_v212_concierge_copy_corrections AS correction
    WHERE correction.order_id = p_order_id FOR UPDATE;
    SELECT correction.* INTO v_existing FROM public.earlybird_v213_concierge_copy_corrections AS correction
    WHERE correction.order_id = p_order_id FOR UPDATE;
    ...
    IF v_order.id IS NULL OR v_order.user_id IS DISTINCT FROM p_owner_id
       ...
       OR v_replay.published_result_hash IS DISTINCT FROM p_expected_published_result_hash THEN
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_SCOPE_OR_CAS_CONFLICT', ERRCODE = 'P0001';
    END IF;
    IF v_fulfillment.order_id IS NULL OR v_fulfillment.status IS DISTINCT FROM 'analysis_in_progress'
       OR v_fulfillment.attempt_count IS DISTINCT FROM 2 THEN
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_FULFILLMENT_SCOPE_CONFLICT', ERRCODE = 'P0001';
    END IF;
    IF EXISTS (
        SELECT 1 FROM public.earlybird_webhook_events AS refund_event
        ...
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_REFUND_REJECTED', ERRCODE = 'P0001';
    END IF;
    IF v_prior.order_id IS NULL
       OR v_prior.result_request_id IS DISTINCT FROM p_result_request_id
       OR v_prior.prior_correction_result_hash IS NULL
       OR v_prior.correction_result_hash IS DISTINCT FROM p_prior_correction_result_hash THEN
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_PRIOR_CORRECTION_CONFLICT', ERRCODE = 'P0001';
    END IF;
    IF v_existing.order_id IS NOT NULL THEN
        IF v_existing.result_request_id IS DISTINCT FROM p_result_request_id
           ...
           OR v_existing.copy_payload IS DISTINCT FROM p_copy_payload THEN
            RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_REPLAY_HASH_CONFLICT', ERRCODE = 'P0001';
        END IF;
    ...
    IF v_result_count IS DISTINCT FROM 16 OR v_high_risk_count IS DISTINCT FROM 2 THEN
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_RESULT_SCOPE_CONFLICT', ERRCODE = 'P0001';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_PRIOR_COPY_CAS_CONFLICT', ERRCODE = 'P0001';
    END IF;
    IF EXISTS (
        ...
        WHERE pg_catalog.jsonb_typeof(item) IS DISTINCT FROM 'object'
           ...
           OR pg_catalog.jsonb_array_length(item->'evidenceTerms') NOT BETWEEN 1 AND 4
           OR pg_catalog.jsonb_typeof(item->'interactionEvidence') IS DISTINCT FROM 'object'
           ...
           OR pg_catalog.jsonb_typeof(item->'review') IS DISTINCT FROM 'object'
           OR item->'review'->>'reviewVersion' IS DISTINCT FROM 'v213-full-evidence-review-v1'
           OR item->'review'->>'rank' IS DISTINCT FROM item->>'rank'
           OR item->'review'->>'overviewChanged' IS DISTINCT FROM 'true'
           OR item->'review'->>'previousOverview' IS NULL
           OR item->'review'->>'nextOverview' IS DISTINCT FROM item->>'oneLineOverview'
           OR item->'review'->>'overviewForm' !~ '^v213-full-review-([1-9]|1[0-6])$'
           OR pg_catalog.jsonb_typeof(item->'review'->'observedEvidenceTerms') IS DISTINCT FROM 'array'
           OR pg_catalog.jsonb_array_length(item->'review'->'observedEvidenceTerms') < 1
           OR pg_catalog.jsonb_typeof(item->'review'->'narrative') IS DISTINCT FROM 'object'
           OR pg_catalog.jsonb_typeof(item->'review'->'narrative'->'previousLines') IS DISTINCT FROM 'array'
           OR pg_catalog.jsonb_typeof(item->'review'->'narrative'->'nextLines') IS DISTINCT FROM 'array'
           OR pg_catalog.jsonb_typeof(item->'review'->'narrative'->'retainedSentenceJustifications') IS DISTINCT FROM 'array'
           OR NOT EXISTS (
                SELECT 1 FROM pg_catalog.jsonb_array_elements_text(item->'review'->'observedEvidenceTerms') AS term
                WHERE pg_catalog.char_length(pg_catalog.btrim(term)) > 0
                  AND pg_catalog.strpos(pg_catalog.lower(item->>'oneLineOverview'), pg_catalog.lower(term)) > 0
           )
           OR (item->>'riskGrade' = 'high_risk' AND (
                ...
                OR (item->'interactionEvidence'->'candidateToTarget'->>'likeObserved') IS DISTINCT FROM 'true'
                   AND (item->'interactionEvidence'->'candidateToTarget'->>'commentObserved') IS DISTINCT FROM 'true'
                   AND (item->'interactionEvidence'->'candidateToTarget'->>'tagObserved') IS DISTINCT FROM 'true'
                   AND (item->'interactionEvidence'->'candidateToTarget'->>'mentionObserved') IS DISTINCT FROM 'true'
                   AND (item->'interactionEvidence'->'targetToCandidate'->>'likeObserved') IS DISTINCT FROM 'true'
                   AND (item->'interactionEvidence'->'targetToCandidate'->>'commentObserved') IS DISTINCT FROM 'true'
                   AND (item->'interactionEvidence'->'targetToCandidate'->>'tagObserved') IS DISTINCT FROM 'true'
                   AND (item->'interactionEvidence'->'targetToCandidate'->>'mentionObserved') IS DISTINCT FROM 'true'
           ))
           OR (item->>'riskGrade' <> 'high_risk' AND pg_catalog.jsonb_array_length(item->'riskAnalysis') <> 0)
    ) THEN
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_PAYLOAD_INVALID', ERRCODE = 'P0001';
    END IF;
    IF (SELECT pg_catalog.count(*) FROM pg_catalog.jsonb_array_elements(p_copy_payload->'rows'))
       ...
       OR (SELECT pg_catalog.count(DISTINCT item->'review'->>'overviewForm') FROM pg_catalog.jsonb_array_elements(p_copy_payload->'rows') AS item) <> 16
    ...
    IF EXISTS (
        ...
        JOIN LATERAL pg_catalog.jsonb_array_elements(v_prior.copy_payload->'rows') AS prior_row
          ON prior_row->>'rank' = item->>'rank'
        WHERE item->'review'->>'previousOverview' IS DISTINCT FROM prior_row->>'oneLineOverview'
           OR item->>'oneLineOverview' IS NOT DISTINCT FROM prior_row->>'oneLineOverview'
           OR item->'review'->'narrative'->'previousLines' IS DISTINCT FROM prior_row->'riskAnalysis'
           OR item->'review'->'narrative'->'nextLines' IS DISTINCT FROM item->'riskAnalysis'
    ) THEN
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_SEMANTIC_DIFF_CONFLICT', ERRCODE = 'P0001';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.jsonb_array_elements(p_copy_payload->'rows') AS item
        CROSS JOIN LATERAL pg_catalog.jsonb_array_elements(
            item->'review'->'narrative'->'retainedSentenceJustifications'
        ) AS justification
        WHERE NOT EXISTS (
            SELECT 1
            FROM pg_catalog.jsonb_array_elements_text(item->'review'->'narrative'->'previousLines') AS prior_line
            WHERE prior_line = justification->>'sentence'
              AND EXISTS (
                  SELECT 1 FROM pg_catalog.jsonb_array_elements_text(
                      item->'review'->'narrative'->'nextLines'
                  ) AS next_line WHERE next_line = prior_line
              )
        )
    ) OR EXISTS (
        SELECT 1
        FROM pg_catalog.jsonb_array_elements(p_copy_payload->'rows') AS item
        WHERE pg_catalog.jsonb_array_length(item->'review'->'narrative'->'retainedSentenceJustifications')
            IS DISTINCT FROM (
           ...
           OR (SELECT pg_catalog.count(DISTINCT justification->>'sentence')
    ...
    IF EXISTS (
        ...
        CROSS JOIN LATERAL pg_catalog.jsonb_array_elements_text(
            item->'review'->'narrative'->'previousLines'
        ) AS prior_line
        WHERE EXISTS (
            SELECT 1 FROM pg_catalog.jsonb_array_elements_text(
                item->'review'->'narrative'->'nextLines'
        ...
        AND NOT EXISTS (
            ...
            WHERE justification->>'sentence' = prior_line
              AND pg_catalog.jsonb_typeof(justification->'observedEvidenceTerms') = 'array'
              AND pg_catalog.jsonb_array_length(justification->'observedEvidenceTerms') > 0
        )
    ) THEN
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_SEMANTIC_DIFF_CONFLICT', ERRCODE = 'P0001';
    END IF;
    IF EXISTS (
        ...
        FROM pg_catalog.jsonb_array_elements(p_copy_payload->'rows') AS item
        CROSS JOIN LATERAL (
            SELECT item->'riskAnalysis'->>1 AS direction_line
        ) AS narrative
        WHERE item->>'riskGrade' = 'high_risk' AND (
            ((item->'interactionEvidence'->'candidateToTarget'->>'likeObserved') = 'true'
                AND NOT (pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'candidate')
                    < pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'target')
                    AND pg_catalog.strpos(narrative.direction_line, '게시물') > 0
                    AND pg_catalog.strpos(narrative.direction_line, '좋아요') > 0))
            OR ((item->'interactionEvidence'->'candidateToTarget'->>'commentObserved') = 'true'
                AND NOT (pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'candidate')
                    < pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'target')
                    AND pg_catalog.strpos(narrative.direction_line, '게시물') > 0
                    AND pg_catalog.strpos(narrative.direction_line, '댓글') > 0))
            OR ((item->'interactionEvidence'->'candidateToTarget'->>'tagObserved') = 'true'
                AND NOT (pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'candidate')
                    < pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'target')
                    AND pg_catalog.strpos(narrative.direction_line, '태그') > 0))
            OR ((item->'interactionEvidence'->'candidateToTarget'->>'mentionObserved') = 'true'
                AND NOT (pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'candidate')
                    < pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'target')
                    AND pg_catalog.strpos(narrative.direction_line, '멘션') > 0))
            OR ((item->'interactionEvidence'->'targetToCandidate'->>'likeObserved') = 'true'
                AND NOT (pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'target')
                    < pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'candidate')
                    AND pg_catalog.strpos(narrative.direction_line, '피드') > 0
                    AND pg_catalog.strpos(narrative.direction_line, '좋아요') > 0))
            OR ((item->'interactionEvidence'->'targetToCandidate'->>'commentObserved') = 'true'
                AND NOT (pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'target')
                    < pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'candidate')
                    AND pg_catalog.strpos(narrative.direction_line, '피드') > 0
                    AND pg_catalog.strpos(narrative.direction_line, '댓글') > 0))
            OR ((item->'interactionEvidence'->'targetToCandidate'->>'tagObserved') = 'true'
                AND NOT (pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'target')
                    < pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'candidate')
                    AND pg_catalog.strpos(narrative.direction_line, '태그') > 0))
            OR ((item->'interactionEvidence'->'targetToCandidate'->>'mentionObserved') = 'true'
                AND NOT (pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'target')
                    < pg_catalog.strpos(narrative.direction_line, item->'subjects'->>'candidate')
                    AND pg_catalog.strpos(narrative.direction_line, '멘션') > 0))
        )
    ) THEN
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_INTERACTION_DIRECTION_CONFLICT', ERRCODE = 'P0001';
    END IF;
    IF EXISTS (
        SELECT 1 FROM pg_catalog.jsonb_array_elements(p_copy_payload->'rows') AS item
        ...
        RAISE EXCEPTION USING MESSAGE = 'CONCIERGE_COPY_V213_ROW_BINDING_CONFLICT', ERRCODE = 'P0001';
    END IF;
    ...
    UPDATE public.analysis_requests SET step_data = pg_catalog.jsonb_set(
        ...
        '{conciergeBootstrap,v213CopyCorrection}',
        pg_catalog.jsonb_build_object('qualityVersion', 'v213-full-evidence-review-v1',
            'correctionResultHash', p_correction_result_hash,
            'priorCorrectionResultHash', p_prior_correction_result_hash,
            'correctedRows', 16, 'correctedHighRiskRows', 2), TRUE)
...
GRANT EXECUTE ON FUNCTION public.correct_earlybird_v213_concierge_copy(UUID, UUID, UUID, TEXT, TEXT, TEXT, TEXT, JSONB)
    TO service_role;
COMMIT;
