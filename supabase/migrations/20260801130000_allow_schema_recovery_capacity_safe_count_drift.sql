-- Checkout freezes paid-order counts, while a consumed preflight can retain a
-- later observed count. Recovery keeps both witnesses immutable and permits
-- their difference only when each independently remains inside the selected
-- paid card and the original capacity partition still holds.
--
-- Recovery previously accepted only ANALYSIS_V2_STAGE_SCHEMA_VALIDATION_ERROR.
-- JOB_ATTEMPTS_EXHAUSTED means the pipeline ran out of retries -- an infrastructure
-- condition such as AI capacity starvation. It says nothing about whether the
-- target is analysable, so it is exactly the class that should be retryable once
-- the underlying capacity is restored. Target-quality failures
-- (SCRAPING_INCOMPLETE_ERROR, ANALYSIS_V2_PROFILE_EVIDENCE_INCOMPLETE, ...) stay
-- in manual_review and are deliberately NOT admitted here.
--
-- The failure receipt must now corroborate the request's own error_message rather
-- than a hardcoded constant, so widening the accepted set can never let a request
-- failing for one reason be recovered on another reason's receipt.

SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '2min';

CREATE OR REPLACE FUNCTION public.recover_earlybird_schema_failed_fulfillment(p_order_id uuid)
...
BEGIN
    ...
    IF NOT FOUND
       ...
       OR v_failed_request.error_message NOT IN (
    ...
    IF v_capacity_rank IS NULL
       ...
       OR v_preflight.target_following_count < 0 THEN
        RAISE EXCEPTION USING
            MESSAGE = 'EARLYBIRD_SCHEMA_FAILURE_RECOVERY_SNAPSHOT_CONFLICT', ERRCODE = 'P0001';
    END IF;
        ...
        IF v_plan_rank >= v_capacity_rank
           AND (
            ...
            RAISE EXCEPTION USING
                MESSAGE = 'EARLYBIRD_SCHEMA_FAILURE_RECOVERY_SNAPSHOT_CONFLICT', ERRCODE = 'P0001';
        END IF;
        IF v_plan_id = v_order.plan_id
           AND (
                v_preflight.target_followers_count > v_card_followers
                OR v_preflight.target_following_count > v_card_following
           ) THEN
            RAISE EXCEPTION USING
                MESSAGE = 'EARLYBIRD_SCHEMA_FAILURE_RECOVERY_SNAPSHOT_CONFLICT', ERRCODE = 'P0001';
        END IF;
