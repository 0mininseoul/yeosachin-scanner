-- Groble 웹훅에는 정가/할인액 필드가 없고 finalAmount(실결제액)만 온다. 지금까지는
-- 이 실결제액이 우리 쪽 expected_amount_krw 와 정확히 같아야만 'accepted' 로 인정했는데,
-- 이러면 100% 할인 쿠폰(0원)이나 부분 할인 쿠폰 결제가 전부 'mismatch' 로 튕겨나간다.
--
-- 정책(사용자 확정): 0 <= 실결제액 <= 정가 는 모두 정상 결제로 인정한다. 정가를
-- 초과하는 금액만 여전히 mismatch 로 남긴다. actual_amount_krw 에는 실제 받은 금액을
-- 그대로 기록하므로(0 또는 할인가), 환불 정산 시 정가가 아니라 실제 수령액을 근거로 삼는다.
--
-- 세 함수 모두 시그니처(파라미터 타입)는 바꾸지 않으므로 CREATE OR REPLACE 가 기존
-- REVOKE ALL / GRANT EXECUTE TO service_role ACL 을 그대로 유지한다.

SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '2min';

-- amount_krw 는 Groble 이 실제로 받은 금액을 그대로 기록하는 감사 로그 컬럼이라
-- 0원(완전 할인 쿠폰)도 유효한 값이다. expected_amount_krw(정가)는 계속 > 0 이어야
-- 하므로 그 CHECK 는 건드리지 않는다.
ALTER TABLE public.earlybird_webhook_events
    DROP CONSTRAINT earlybird_webhook_events_amount_check;
ALTER TABLE public.earlybird_webhook_events
    ADD CONSTRAINT earlybird_webhook_events_amount_check CHECK (amount_krw >= 0);
