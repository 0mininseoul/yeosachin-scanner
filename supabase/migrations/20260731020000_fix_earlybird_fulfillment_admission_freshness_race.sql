...
DECLARE
    ...
    v_input_hash TEXT;
BEGIN
    IF p_order_id IS NULL OR p_lease_token IS NULL
    ...
    -- Preserve the original request/order/preflight idempotency family before
    -- considering the current lease or admission clock.
    IF v_fulfillment.request_id IS NOT NULL
       ...
       OR v_preflight.consumed_request_id IS NOT NULL THEN
        IF v_fulfillment.request_id IS NULL
           ...
           OR v_preflight.consumed_request_id IS DISTINCT FROM v_fulfillment.request_id THEN
            ...
            SET status = 'manual_review', lease_token = NULL, lease_expires_at = NULL,
                last_error_code = 'REQUEST_CONFLICT', last_error_at = v_now,
        ...
        SET status = CASE WHEN v_request.status = 'completed' THEN 'completed' ELSE 'analysis_in_progress' END,
            ...
            completed_at = CASE WHEN v_request.status = 'completed' THEN v_now ELSE NULL END,
            updated_at = v_now
        WHERE fulfillment.order_id = p_order_id;
        RETURN QUERY SELECT p_order_id,
            CASE WHEN v_request.status = 'completed' THEN 'completed'::TEXT ELSE 'analysis_in_progress'::TEXT END,
            v_request.id, FALSE, v_initial_job_key;
        RETURN;
    END IF;
    ...
    -- Keep every non-clock proof immutable.  A changed paid-order, identity,
    -- target/exclusion, or admission state remains a manual snapshot conflict.
    IF v_order.status <> 'paid'
       ...
       OR NOT public.analysis_v2_valid_policy_versions_snapshot(v_preflight.policy_versions_snapshot) THEN
        UPDATE public.earlybird_fulfillments AS fulfillment
        SET status = 'manual_review', lease_token = NULL, lease_expires_at = NULL,
            last_error_code = 'SNAPSHOT_CONFLICT', last_error_at = v_now,
            manual_review_at = v_now, updated_at = v_now
        WHERE fulfillment.order_id = p_order_id;
        RETURN QUERY SELECT p_order_id, 'manual_review'::TEXT, NULL::UUID, FALSE, NULL::TEXT;
        RETURN;
    ...
    IF v_order.plan_id NOT IN ('basic', 'standard')
       ...
       OR v_preflight.target_following_count > (v_selected_card->'relationshipCapacity'->>'following')::INTEGER THEN
        UPDATE public.earlybird_fulfillments AS fulfillment
        SET status = 'manual_review', lease_token = NULL, lease_expires_at = NULL,
            last_error_code = 'PLAN_NOT_ALLOWED', last_error_at = v_now,
            manual_review_at = v_now, updated_at = v_now
        WHERE fulfillment.order_id = p_order_id;
        RETURN QUERY SELECT p_order_id, 'manual_review'::TEXT, NULL::UUID, FALSE, NULL::TEXT;
        RETURN;
    ...
    -- This is intentionally the only retryable branch. NULL and future
    -- timestamps remain corrupt/conflicting snapshots, never a retry bypass.
    IF v_preflight.admission_refreshed_at IS NOT NULL
       AND v_preflight.admission_refreshed_at < v_now - INTERVAL '2 minutes' THEN
        UPDATE public.earlybird_fulfillments AS fulfillment
        SET status = 'retryable_failure', lease_token = NULL, lease_expires_at = NULL,
            ...
            last_error_at = v_now, manual_review_at = NULL, updated_at = v_now
        WHERE fulfillment.order_id = p_order_id;
    ...
    IF EXISTS (
        ...
        UPDATE public.earlybird_fulfillments AS fulfillment
        SET status = 'manual_review', lease_token = NULL, lease_expires_at = NULL,
            last_error_code = 'ACTIVE_REQUEST_CONFLICT', last_error_at = v_now,
            manual_review_at = v_now, updated_at = v_now
        WHERE fulfillment.order_id = p_order_id;
        RETURN QUERY SELECT p_order_id, 'manual_review'::TEXT, NULL::UUID, FALSE, NULL::TEXT;
        RETURN;
    ...
    INSERT INTO public.analysis_requests (
        ...
        v_request_id, v_order.user_id, v_order.target_instagram_id, 'male', 'pending', 0,
        '분석 대기 중...', 'pending', '{}'::JSONB, '{}'::JSONB, v_order.plan_id,
        TRUE, 'earlybird:' || pg_catalog.lower(v_order.id::TEXT), 'v2', v_preflight.id,
        v_order.excluded_instagram_id, v_order.exclusion_decision, 'production',
        v_preflight.capacity_required_plan_id, v_preflight.required_plan_id,
        v_order.plan_id, v_preflight.launch_status_snapshot, v_preflight.plan_cards_snapshot,
    ...
    v_input_hash := pg_catalog.encode(extensions.digest(pg_catalog.convert_to(
        ...
        || v_initial_job_key, 'UTF8'), 'sha256'), 'hex');
    INSERT INTO public.analysis_pipeline_jobs(
        request_id, job_key, track, kind, batch, input_hash, required_job_keys
    ) VALUES (v_request_id, v_initial_job_key, 'coordinator', 'bootstrap', NULL,
        v_input_hash, '{}'::TEXT[])
    ...
    IF NOT FOUND OR v_job.track <> 'coordinator' OR v_job.kind <> 'bootstrap'
       OR v_job.batch IS NOT NULL OR v_job.input_hash <> v_input_hash
    ...
    SET status = 'analysis_in_progress', result_request_id = v_request_id, updated_at = v_now
    ...
    SET status = 'analysis_in_progress', request_id = v_request_id,
        ...
        last_error_code = NULL, last_error_at = NULL, updated_at = v_now
    WHERE fulfillment.order_id = p_order_id;
...
BEGIN
    IF p_order_id IS NULL OR p_expected_manual_review_at IS NULL THEN
        RAISE EXCEPTION USING MESSAGE = 'EARLYBIRD_FRESHNESS_RECOVERY_INVALID', ERRCODE = 'P0001';
    END IF;
    SELECT earlybird_order.* INTO v_order FROM public.earlybird_orders AS earlybird_order
    WHERE earlybird_order.id = p_order_id FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION USING MESSAGE = 'EARLYBIRD_FRESHNESS_RECOVERY_NOT_FOUND', ERRCODE = 'P0001';
    END IF;
    ...
    IF v_fulfillment.manual_review_at IS DISTINCT FROM p_expected_manual_review_at THEN
        RAISE EXCEPTION USING MESSAGE = 'EARLYBIRD_FRESHNESS_RECOVERY_CAS_MISMATCH', ERRCODE = 'P0001';
    END IF;
    IF v_order.status <> 'paid' OR v_order.seller_reference_confirmed_at IS NULL
       OR v_order.payment_id IS NULL OR v_order.actual_amount_krw IS NULL
       OR v_order.actual_amount_krw < 0 OR v_order.actual_amount_krw > v_order.expected_amount_krw
       OR v_order.actual_groble_product_id IS DISTINCT FROM v_order.expected_groble_product_id
       OR v_preflight.user_id IS DISTINCT FROM v_order.user_id
       -- A stale manual conflict may have outlived the preflight's immutable
       -- execution TTL.  Advance will rebind that paid order after this CAS;
