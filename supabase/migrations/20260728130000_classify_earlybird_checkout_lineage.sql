-- Checkout lineage is classification-only at launch.  A provider completion
-- event identifies a product and verified buyer, but not an application intent;
-- replacing or cancelling an unresolved lineage would make that attribution
-- ambiguous.  Keep every stored order immutable and return stable, safe-to-show
-- classifications instead.
--
-- `STALE_PRICING_LINEAGE` is derived exclusively from the stored v1 snapshot.
-- Every other immutable pricing snapshot is `SUPERSEDED_LINEAGE`.  Do not use
-- mutable preflight state, clock time, or caller-supplied values for this label.
...
BEGIN
    ...
    IF p_pricing_version IS NULL
       ...
       OR p_disclosure_text IS DISTINCT FROM '현재 얼리버드 기간에는 즉시 자동 판독이 아닌, 결제 완료 후 24시간 이내 판독 결과를 제공합니다.'
    ...
    -- original immutable payment snapshot even while pricing callers roll over.
    ...
    -- Never cancel, supersede, expire, or replace a live checkout.  The
    -- original order remains the only provider-attributable payment intent.
