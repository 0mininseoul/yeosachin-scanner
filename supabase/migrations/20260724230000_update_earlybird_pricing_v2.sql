-- Generated with `supabase migration new` as UTC version 20260724174955, then
-- ordered after the already-applied 20260724203500 head and the reviewed,
-- merge-pending 20260724220000 migration. This preserves ordinary append-only
-- `db push` behavior without using `--include-all`.
--
-- Switch newly issued earlybird snapshots to pricing v2:
-- Basic 13,900 -> 6,900 KRW and Standard 19,900 -> 9,900 KRW.
--
-- This is an expand migration for a migration-before-code rollout:
-- * v1 callers may finish creating an order from an existing v1 preflight while
--   old application instances drain.
-- * v2 callers must use a v2 preflight. A v2 caller holding a v1 preflight gets
--   an explicit refresh boundary instead of silently mutating its snapshot.
-- * an already-issued v1 payment_pending order remains replayable after the
--   application switches to v2; its immutable expected amount and pricing
--   version stay untouched.
-- * paid/cancelled orders and webhook audit history are never rewritten.
...
BEGIN
    ...
    END IF;
    IF v_preflight.pricing_snapshot->p_plan_id->>'status' <> 'quoted'
