-- MIGRATION_PREDECESSOR=20260815130000
-- Recover only the verified shape where one paid concierge order retained one
-- completed, reconciled SENARY g1 fresh-admission provider run on its current
-- preflight, then a released g2 dispatch cleared the admission snapshot before
-- any g2 provider work started.  This uses that immutable g1 ledger and the
-- unchanged preflight snapshot to consume the already-created preflight; it
-- never rebinds, reserves, or starts a provider run.
BEGIN;
...
BEGIN
    ...
    -- A response lost after commit is an idempotent replay only when it names
    -- this exact consumed preflight and its sole recorded request.
    ...
    -- This is the sole recovery transition.  The successful g1 ledger proves
    -- the target was collected; g2 has no provider entry and therefore cannot
    -- be recollected.  The unchanged preflight snapshot is copied back into
    -- the admission fields solely to create the queued request under its lease.
...
COMMENT ON FUNCTION public.recover_exact_earlybird_generation_two_pending_idle(
    UUID, UUID
) IS 'Exact paid concierge recovery for a completed SENARY g1 and g2 pending-idle zero-g2-run checkpoint; resumes request creation without rebind or recollection.';
