-- Every authenticated /betatest visitor may be enrolled only through a
-- narrow self-service boundary. The durable grant row remains the sole
-- downstream authorization witness so existing service-only locks/rechecks
-- stay valid. A service-only policy rollback disables automatic rows in the
-- same transaction without affecting operator grants.
...
COMMENT ON COLUMN public.analysis_beta_access_grants.grant_source IS
    'operator grants survive grant_only rollback; automatic rows are server-created only for all_authenticated policy.';
...
COMMENT ON TABLE public.analysis_beta_access_policy IS
    'Service-owned betatest enrollment policy. Runtime gate remains the immediate kill switch.';
...
BEGIN
    ...
    IF p_enabled = TRUE AND p_expires_at IS NOT NULL AND p_expires_at <= v_now THEN
        RAISE EXCEPTION USING MESSAGE = 'ANALYSIS_BETA_GRANT_INVALID', ERRCODE = 'P0001';
    END IF;

    INSERT INTO public.analysis_beta_access_grants (
        user_id, enabled, expires_at, audit_reference_hash, grant_source,
        granted_at, updated_at
    ) VALUES (
        p_user_id, p_enabled, p_expires_at, p_audit_reference_hash, 'operator',
        v_now, v_now
...
CREATE OR REPLACE FUNCTION public.set_analysis_beta_access_policy(
    p_mode TEXT
)
RETURNS TEXT
LANGUAGE plpgsql
...
BEGIN
    ...
    SELECT policy_row.mode INTO v_mode
    ...
    SET mode = p_mode,
        generation = policy_row.generation + CASE
            WHEN policy_row.mode IS DISTINCT FROM p_mode THEN 1 ELSE 0 END,
    ...
    IF p_mode = 'grant_only' THEN
        ...
        SET enabled = FALSE,
            updated_at = pg_catalog.clock_timestamp()
        WHERE grant_row.grant_source = 'automatic'
...
REVOKE ALL ON FUNCTION public.set_analysis_beta_access_policy(TEXT)
    FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.set_analysis_beta_access_policy(TEXT)
    TO service_role;

CREATE OR REPLACE FUNCTION public.enroll_analysis_beta_authenticated_user()
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
    ...
    FOR SHARE;
    SELECT policy_row.mode INTO v_policy_mode
    FROM public.analysis_beta_access_policy AS policy_row
    WHERE policy_row.singleton = TRUE
    FOR SHARE;
    IF v_gate_enabled IS DISTINCT FROM TRUE THEN
        RETURN FALSE;
    END IF;
    ...
    IF v_policy_mode = 'grant_only' THEN
        RETURN EXISTS (
            ...
            WHERE grant_row.user_id = v_user_id
              AND grant_row.grant_source = 'operator'
              AND grant_row.enabled = TRUE
              AND (
                    grant_row.expires_at IS NULL
                    OR grant_row.expires_at > pg_catalog.clock_timestamp()
              )
        );
    END IF;
    IF v_policy_mode IS DISTINCT FROM 'all_authenticated' THEN
        RETURN FALSE;
    END IF;

    PERFORM pg_catalog.pg_advisory_xact_lock(
        pg_catalog.hashtextextended(
            'analysis-beta-grant:' || pg_catalog.lower(v_user_id::TEXT), 0
        )
    );
    PERFORM users.id FROM public.users AS users
    WHERE users.id = v_user_id FOR KEY SHARE;
    IF NOT FOUND THEN
        RETURN FALSE;
    END IF;
    ...
    IF NOT FOUND THEN
        INSERT INTO public.analysis_beta_access_grants (
            ...
            pg_catalog.encode(extensions.digest(
                'automatic-betatest-enrollment-v1:' || v_user_id::TEXT,
                'sha256'
            ), 'hex'),
    ...
    IF v_grant.grant_source = 'automatic'
       AND v_grant.enabled = TRUE
       AND v_grant.expires_at IS NULL THEN
        RETURN TRUE;
    END IF;
...
REVOKE ALL ON FUNCTION public.enroll_analysis_beta_authenticated_user()
    FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.enroll_analysis_beta_authenticated_user()
    TO authenticated;

COMMENT ON FUNCTION public.enroll_analysis_beta_authenticated_user() IS
    'Authenticated caller-only enrollment. It never accepts a user identity and creates/reactivates automatic grants only while gate and all_authenticated policy are enabled.';
