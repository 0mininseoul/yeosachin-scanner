-- v2.9 is a forward-only performance policy. Existing v2.7/v2.8 captures remain
-- readable through the immutable predecessor function; new capture arms only for
-- the exact v2.9 scheduler-backed snapshot.
...
AS $$
    SELECT p_snapshot IS NOT NULL
       ...
       AND p_snapshot->>'aiStage' = 'ai-stage-policy-v2.9'
...
-- Keep the full old reader intact for historical lineages; a v2.9 source is
-- recognized only by all four exact policy keys before any Apify-led replay is
-- constructed. The old reader is not an ambient-policy fallback for v2.9.
ALTER FUNCTION public.read_analysis_v2_replay_capture_source(TEXT, UUID)
    RENAME TO read_analysis_v2_replay_capture_source_v28_legacy;

CREATE FUNCTION public.read_analysis_v2_replay_capture_source(
...
BEGIN
    ...
    WHERE pg_catalog.lower(request.target_instagram_id) = v_target
      ...
      AND request.policy_versions_snapshot =
          '{"pipeline":"v2","risk":"risk-policy-v2.4","aiStage":"ai-stage-policy-v2.9","scheduler":"ai-scheduler-v1"}'::JSONB
    ORDER BY request.completed_at DESC, request.id DESC
    LIMIT 1;

    IF NOT FOUND THEN
        RETURN public.read_analysis_v2_replay_capture_source_v28_legacy(
            p_target_username, p_request_id
        );
    END IF;
...
REVOKE ALL ON FUNCTION public.read_analysis_v2_replay_capture_source_v28_legacy(TEXT, UUID)
    FROM PUBLIC, anon, authenticated, service_role;

COMMENT ON FUNCTION public.read_analysis_v2_replay_capture_source(TEXT, UUID) IS
    'Read-only exact v2.9 Standard lineage with an immutable v2.7/v2.8 source-reader fallback; never substitutes a different AI policy.';
