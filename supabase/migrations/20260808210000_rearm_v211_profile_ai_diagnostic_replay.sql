-- MIGRATION_PREDECESSOR=20260808200000
-- The first paid concierge replay passed relationship collection and reached
-- profile AI, but the worker collapsed the concrete runtime failure into the
-- generic job-handler code. Preserve that terminal request and admit exactly
-- one r7 replay after the diagnostic worker has been deployed. Successful
-- source runs remain adoptable; a missing or aborted exact source run may be
-- collected fresh under the already-paid entitlement.
BEGIN;
    ...
    IF pg_catalog.to_regclass('supabase_migrations.schema_migrations') IS NOT NULL THEN
        EXECUTE $sql$
            SELECT EXISTS (
                ...
                WHERE version = '20260808200000'
            )
        $sql$ INTO v_predecessor_present;
    END IF;
    IF NOT v_predecessor_present THEN
        RAISE EXCEPTION USING
            MESSAGE = 'EARLYBIRD_V211_PROFILE_AI_DIAGNOSTIC_PREDECESSOR_MISSING',
            ERRCODE = 'P0001';
    END IF;
END;
$migration$;
...
ALTER TABLE public.earlybird_v211_profile_ai_diagnostic_replays
    ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.earlybird_v211_profile_ai_diagnostic_replays
    FORCE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.earlybird_v211_profile_ai_diagnostic_replays
...
AS $$
    SELECT EXISTS (
        ...
        JOIN public.earlybird_v211_concierge_replays AS concierge
          ON concierge.order_id = diagnostic.order_id
         AND concierge.original_failed_request_id =
                diagnostic.original_failed_request_id
         AND concierge.rearmed_preflight_id = diagnostic.failed_preflight_id
         AND concierge.second_relationship_failed_request_id IS NOT NULL
        JOIN public.earlybird_schema_failure_recoveries AS recovery
          ON recovery.order_id = diagnostic.order_id
         AND recovery.failed_request_id = diagnostic.original_failed_request_id
        JOIN public.earlybird_orders AS earlybird_order
          ON earlybird_order.id = diagnostic.order_id
        JOIN public.analysis_requests AS original_request
          ON original_request.id = diagnostic.original_failed_request_id
        JOIN public.analysis_requests AS profile_ai_request
          ON profile_ai_request.id = diagnostic.profile_ai_failed_request_id
        JOIN public.analysis_preflights AS failed_preflight
          ON failed_preflight.id = diagnostic.failed_preflight_id
        JOIN public.analysis_preflights AS current_preflight
          ON current_preflight.id = diagnostic.rearmed_preflight_id
        ...
        WHERE diagnostic.order_id = p_order_id
          ...
          AND diagnostic.rearmed_preflight_id = p_current_preflight_id
          ...
          AND profile_ai_request.user_id = earlybird_order.user_id
          AND profile_ai_request.preflight_id = failed_preflight.id
          AND profile_ai_request.pipeline_version = 'v2'
          AND profile_ai_request.status = 'failed'
          AND profile_ai_request.error_message =
                'ANALYSIS_V2_JOB_HANDLER_FAILED'
          AND profile_ai_request.idempotency_key =
                'earlybird:' || pg_catalog.lower(earlybird_order.id::TEXT) || '.r3'
          AND failed_preflight.user_id = earlybird_order.user_id
          AND failed_preflight.status = 'consumed'
          AND failed_preflight.consumed_request_id = profile_ai_request.id
          ...
          AND 1 = (
              ...
              WHERE receipt.request_id = profile_ai_request.id
          ...
          AND 6 = (
              ...
              WHERE adoption.request_id = profile_ai_request.id
                AND adoption.source_request_id = original_request.id
          )
          AND 5 = (
              SELECT pg_catalog.count(*)
              FROM public.analysis_v2_provider_runs AS provider_run
              WHERE provider_run.request_id = profile_ai_request.id
          ...
          AND NOT EXISTS (
              ...
              WHERE cleanup.request_id = profile_ai_request.id
                AND cleanup.completed_at IS NULL
          )
          AND 1 = (
              SELECT pg_catalog.count(*)
              FROM public.analysis_v2_ai_attempts AS attempt
              WHERE attempt.request_id = profile_ai_request.id
          ...
          AND 7 = (
              ...
              WHERE operation.request_id = profile_ai_request.id
          )
          AND 6 = (
              SELECT pg_catalog.count(*)
              FROM public.analysis_v2_scheduler_operations AS operation
              WHERE operation.request_id = profile_ai_request.id
                AND operation.job_key = 'track:profile-ai:batch:2'
          ...
          AND NOT EXISTS (
              ...
              WHERE lease.request_id = profile_ai_request.id
          )
          AND NOT EXISTS (
              SELECT 1 FROM public.analysis_v2_ai_result_checkpoints AS checkpoint
              WHERE checkpoint.request_id = profile_ai_request.id
          )
          AND 7 = (
              SELECT pg_catalog.count(*)
              FROM public.analysis_v2_provider_runs AS provider_run
...
REVOKE ALL ON FUNCTION public.earlybird_v211_profile_ai_diagnostic_replay_ready(
    UUID, UUID, UUID, UUID
) FROM PUBLIC, anon, authenticated, service_role;
...
DECLARE
    ...
    v_definition TEXT;
    v_rewritten TEXT;
    v_expected_old_hash CONSTANT TEXT := 'f26a61eb5e4abe794deabde9cba457f2';
    v_old TEXT := $old$                                  OR (
...
BEGIN
    ...
    IF pg_catalog.md5(v_definition) <> v_expected_old_hash
       ...
       OR pg_catalog.strpos(v_definition, 'source_run.status = ''aborted''') = 0 THEN
        RAISE EXCEPTION
            'EARLYBIRD_V211_PROFILE_AI_READINESS_OLD_SHAPE_MISMATCH';
    END IF;
    ...
    IF v_rewritten = v_definition
       ...
       OR pg_catalog.strpos(v_rewritten, ') || ''.r7''') = 0
       OR pg_catalog.strpos(v_rewritten, 'source_run.status = ''aborted''') = 0 THEN
        RAISE EXCEPTION
            'EARLYBIRD_V211_PROFILE_AI_READINESS_REWRITE_MISMATCH';
    END IF;
    EXECUTE v_rewritten;
END;
$readiness_patch$;
...
DECLARE
    ...
    v_definition TEXT;
    v_rewritten TEXT;
    v_expected_old_hash CONSTANT TEXT := '787cf9dddcb3a10f69f257f8e7f18219';
...
BEGIN
    ...
    IF pg_catalog.md5(v_definition) <> v_expected_old_hash
       ...
       OR pg_catalog.strpos(
        ...
        RAISE EXCEPTION
            'ANALYSIS_V2_V211_PROFILE_AI_EXACT_RESOLVER_OLD_SHAPE_MISMATCH';
    END IF;
    ...
    IF v_rewritten = v_definition
       ...
       OR pg_catalog.strpos(
        ...
        RAISE EXCEPTION
            'ANALYSIS_V2_V211_PROFILE_AI_EXACT_RESOLVER_REWRITE_MISMATCH';
    END IF;
    EXECUTE v_rewritten;
END;
...
CREATE FUNCTION public.rearm_earlybird_v211_profile_ai_diagnostic_replay(
    p_order_id UUID,
...
DECLARE
    ...
    v_existing public.earlybird_v211_profile_ai_diagnostic_replays%ROWTYPE;
...
BEGIN
    IF p_order_id IS NULL OR p_expected_failed_request_id IS NULL
       OR p_expected_manual_review_at IS NULL THEN
        RAISE EXCEPTION USING
            MESSAGE = 'EARLYBIRD_V211_PROFILE_AI_DIAGNOSTIC_REPLAY_INVALID',
            ERRCODE = 'P0001';
    END IF;
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION USING
            MESSAGE = 'EARLYBIRD_V211_PROFILE_AI_DIAGNOSTIC_REPLAY_INELIGIBLE',
            ERRCODE = 'P0001';
    END IF;
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION USING
            MESSAGE = 'EARLYBIRD_V211_PROFILE_AI_DIAGNOSTIC_REPLAY_CONFLICT',
            ERRCODE = 'P0001';
    END IF;
    ...
    FROM public.earlybird_v211_profile_ai_diagnostic_replays AS audit
    ...
    IF FOUND THEN
        ...
        IF v_existing.profile_ai_failed_request_id
        ...
        RETURN QUERY SELECT
            ...
            v_existing.profile_ai_failed_request_id;
        RETURN;
    END IF;
    ...
    IF v_order.id IS NULL
       ...
       OR v_concierge.rearmed_preflight_id IS DISTINCT FROM v_preflight.id
       OR v_concierge.expected_fulfillment_attempt_count <> 1
       ...
       OR v_request.idempotency_key IS DISTINCT FROM
            ('earlybird:' || pg_catalog.lower(v_order.id::TEXT) || '.r3')
       ...
       OR 1 <> (
            ...
            FROM public.analysis_pipeline_jobs AS job
            WHERE job.request_id = v_request.id
              AND job.job_key = 'track:profile-ai:batch:2'
       ...
       OR 1 <> (
            ...
            FROM public.analysis_v2_recovery_provider_run_adoptions AS adoption
            WHERE adoption.request_id = v_request.id
              AND adoption.job_key = 'track:profiles:batch:0'
              AND adoption.source_job_key = 'track:profiles:batch:0'
       )
       OR 1 <> (
            SELECT pg_catalog.count(*)
            FROM public.analysis_v2_recovery_provider_run_adoptions AS adoption
            WHERE adoption.request_id = v_request.id
              AND adoption.job_key = 'track:profiles:batch:1'
              AND adoption.source_job_key = 'track:profiles:batch:1'
       )
       OR 2 <> (
            SELECT pg_catalog.count(*)
            FROM public.analysis_v2_recovery_provider_run_adoptions AS adoption
       ...
       OR 2 <> (
            ...
            FROM public.analysis_v2_recovery_provider_run_adoptions AS adoption
            WHERE adoption.request_id = v_request.id
              AND adoption.job_key = 'track:target-evidence:collect'
              AND adoption.source_job_key = 'track:target-evidence:collect'
       )
       OR 5 <> (
            SELECT pg_catalog.count(*)
            FROM public.analysis_v2_provider_runs AS provider_run
       ...
       OR EXISTS (
            ...
            WHERE next_preflight.user_id = v_order.user_id
              AND next_preflight.idempotency_key = v_base_preflight_key || '.r7'
       ) THEN
        RAISE EXCEPTION USING
            MESSAGE = 'EARLYBIRD_V211_PROFILE_AI_DIAGNOSTIC_REPLAY_INELIGIBLE',
            ERRCODE = 'P0001';
    ...
    INSERT INTO public.analysis_preflights(
        ...
        v_new_preflight_id, v_order.user_id, v_base_preflight_key || '.r7',
...
REVOKE ALL ON FUNCTION public.rearm_earlybird_v211_profile_ai_diagnostic_replay(
    UUID, UUID, TIMESTAMP WITH TIME ZONE
) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.rearm_earlybird_v211_profile_ai_diagnostic_replay(
    UUID, UUID, TIMESTAMP WITH TIME ZONE
) TO service_role;

COMMENT ON FUNCTION public.rearm_earlybird_v211_profile_ai_diagnostic_replay(
    UUID, UUID, TIMESTAMP WITH TIME ZONE
) IS
    'Operator-only r7 diagnostic replay for the first paid v2.11 profile-AI incident.';
...
DECLARE
    ...
    v_exact TEXT :=
        ...
        || 'uuid,text,uuid,text,text,text,text,text,numeric)';
    v_rearm TEXT :=
        'public.rearm_earlybird_v211_profile_ai_diagnostic_replay('
        || 'uuid,uuid,timestamp with time zone)';
BEGIN
    IF pg_catalog.strpos(
       ...
       OR pg_catalog.has_function_privilege('service_role', v_exact, 'EXECUTE') THEN
        RAISE EXCEPTION
            'EARLYBIRD_V211_PROFILE_AI_DIAGNOSTIC_FINAL_GUARD_MISMATCH';
    END IF;
END;
$final_guard$;
