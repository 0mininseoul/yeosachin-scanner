...
This path reduces vendor spend but cannot make Instagram suspension risk zero. A rate-limit signal opens a process cooldown, and challenge or authentication failures quarantine the account until an operator restart/recovery. Paid Analysis V2 keeps the profile path fail-closed after such a failure; the durable Apify fallback remains available only to beta/legacy or explicitly selected public routes. Use only the designated disposable account and keep the kill switch closed until a canary is reviewed.
...
### Direct public-profile primary (legacy/explicit routes)
...
The legacy/explicit public route uses `selfhosted` for direct, unauthenticated target preflight and profile reads. A classified public-provider failure may enter an Apify profile-summary fallback. Its durable billing ceiling remains `$0.0026`, while a `$0.0030` Actor execution cutoff prevents Apify from aborting when the single `$0.0026` profile charge lands exactly on the runtime boundary; an explicit public `not_found` result does not fallback. The initial preflight and each fresh-admission generation reserve distinct PII-free operation rows in `analysis_preflight_provider_runs`. A retry in the same generation resumes its stored run ID, while a fresh generation never reuses the initial preflight snapshot. During migration-first rollout, a generation RPC atomically adopts only a legacy-worker row reserved at or after that generation's `admission_requested_at`; older initial rows are never adopted. If the generation row already exists, a legacy worker's attempt to insert the old operation fails before Actor start. Authenticated usage is reconciled no earlier than 30 seconds after each run settles. The initial fallback and one fresh-generation fallback therefore have independent `$0.0026` ceilings and can cost at most `$0.0052` together. Additional fresh generations, when allowed by the admission state machine, each have their own bounded row and ceiling. An expired or abandoned preflight retains every operation row until all usage is reconciled. This legacy path never supplies an Instagram login cookie or session.

For paid Analysis V2 with `selfhosted_auth`, the target preflight and checkout-time `fresh_admission` call the authenticated worker from the first attempt, and the full target profile plus candidate profile batches use the same worker identity. A classified auth-worker failure is terminal for that operation and does not open the public or Apify profile fallback. The beta route remains Apify-only; the public route above is retained for rollback and explicit operator diagnostics.
