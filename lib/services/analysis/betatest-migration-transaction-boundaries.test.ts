...
describe('pending betatest migration transaction boundaries', () => {
    it.each(pendingMigrationFiles)(
        '%s is self-transactional for Supabase CLI statement execution',
        file => {
            ...
            const transactionScopedStatement = sql.match(
                /\b(?:SET\s+LOCAL|LOCK\s+TABLE)\b/
            );
            ...
            expect(sql, `${file} must commit as its final executable statement`)
                .toMatch(/COMMIT\s*;$/);
        }
    );
});
