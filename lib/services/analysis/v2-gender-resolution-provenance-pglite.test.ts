...
describe('gender resolver provenance migration', () => {
    ...
    it('keeps owner summary limited to gender buckets without failure aggregates', async () => {
            ...
            `SELECT public.analysis_v2_result_summary_json(summary) AS summary
             FROM (
                SELECT (
                    ...
                    10, 10, 10, 10, 5, 4, 1, 4,
                    1, 2, 1, 0, 0, 0, 0, FALSE, 'risk-v1'
                )::public.analysis_v2_result_summaries AS summary
             ) AS source`
        )).rows[0].summary;

        expect(result.genderStats).toEqual({ male: 1, female: 2, unknown: 1 });
        expect(result).not.toHaveProperty('fetchUnavailableMutuals');
        expect(result).not.toHaveProperty('mediaUnavailableMutuals');
        expect(result).not.toHaveProperty('analysisUnavailableMutuals');
        expect(result).not.toHaveProperty('successfullyScreenedMutuals');
    });
    ...
    it('refreshes request-level resolver and partial-media metrics idempotently', async () => {
        await db.query(
            `INSERT INTO public.analysis_v2_ai_attempts(
                request_id, stage, usage_metadata_status
            ) VALUES
                ($1, 'genderResolution', 'complete'),
                ($1, 'genderResolution', 'missing')`,
            [REQUEST_V27]
        );
        await db.query(
        ...
        const metrics = (await db.query<{
            ...
            `SELECT screened_count, baseline_unknown_count, final_unknown_count,
                    applied_count, cutoff_count, partial_media_accepted_candidate_count,
