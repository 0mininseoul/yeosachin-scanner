...
describe('analysis V2 scheduler policy snapshot reader migration', () => {
    it('exposes only a validated V2 snapshot to the service-role worker', () => {
        ...
        expect(migration).toContain('analysis_v2_valid_policy_versions_snapshot_v2');
        expect(migration).toContain('analysis_v2_policy_validator_contract_version');
        expect(migration).toContain(
            "v_contract_version IS DISTINCT FROM 'analysis-v2-policy-validator-v2'"
        );
