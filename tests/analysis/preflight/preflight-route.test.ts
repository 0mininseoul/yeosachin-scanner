...
vi.mock('@/lib/services/analysis/preflight', async (importOriginal) => {
    const actual = await importOriginal<typeof import('../../../lib/services/analysis/preflight')>();
    return {
        ...actual,
...
vi.mock('@/lib/services/analysis/preflight-tasks', async (importOriginal) => {
    const actual = await importOriginal<typeof import('../../../lib/services/analysis/preflight-tasks')>();
    return {
        ...actual,
...
vi.mock('@/lib/services/analysis/anonymous-preflight', async (importOriginal) => {
    const actual = await importOriginal<typeof import('../../../lib/services/analysis/anonymous-preflight')>();
    return {
        ...actual,
...
vi.mock('@/lib/services/analysis/betatest-access', async importOriginal => ({
    ...(await importOriginal<typeof import('../../../lib/services/analysis/betatest-access')>()),
...
import { AnonymousPreflightClaimInvalidError } from '../../../lib/services/analysis/anonymous-preflight';
import {
...
import { PreflightTaskEnqueueError } from '../../../lib/services/analysis/preflight-tasks';
...
function postRequest(
    ...
    options: { deviceId?: string | null; captureToken?: string } = {},
) {
    ...
    if (options.captureToken) headers.set('X-Landing-Lead-Capture-Token', options.captureToken);
...
describe('preflight owner routes', () => {
    beforeEach(() => {
        ...
        mocks.landingCaptureBind.mockResolvedValue({
            bound: true,
            repaired: false,
            journeyId: '523e4567-e89b-42d3-a456-426614174000',
        });
    ...
    it('lets a claim holder read anonymous eligibility and exact stored prices', async () => {
        ...
        expect(byPlan.basic.price.amountKrw).toBe(9_900);
        expect(byPlan.standard.price.amountKrw).toBe(19_900);
    });
    ...
    it('captures and binds the exact authenticated target before acknowledging preflight', async () => {
        const response = await createPreflight(postRequest(
            { targetInstagramId: 'Target.Name' },
            'preflight-key-000000000000',
            undefined,
            {
                captureToken: 'v1.opaque.signed-token',
            },
        ));

        expect(response.status).toBe(202);
        expect(mocks.landingCaptureBind).toHaveBeenCalledWith(
            expect.anything(),
            expect.objectContaining({
                preflightId,
                targetInstagramId: 'target.name',
                landingCaptureToken: 'v1.opaque.signed-token',
                anonymousDeviceId: '123e4567-e89b-42d3-a456-426614174000',
                authUserId: userId,
            }),
        );
    ...
    it('rejects authenticated preflight without a stable browser identifier', async () => {
        const response = await createPreflight(postRequest(
            { targetInstagramId: 'Target.Name' },
            'preflight-key-000000000000',
            undefined,
        ...
        await expect(response.json()).resolves.toMatchObject({ code: 'DEVICE_ID_REQUIRED' });
        expect(mocks.store.createOrReplay).not.toHaveBeenCalled();
        expect(mocks.landingCaptureBind).not.toHaveBeenCalled();
    });
        ...
        const response = await createPreflight(new Request(
            'https://example.com/api/analysis/preflight?channel=betatest',
            {
                ...
                headers: {
                    ...
                    'x-anonymous-device-id': '123e4567-e89b-42d3-a456-426614174000',
