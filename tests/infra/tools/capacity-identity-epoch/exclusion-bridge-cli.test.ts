...
} from '../../../../scripts/capacity-identity-epoch/exclusion-bridge';
import { reservationResourceDigest, reservationResourceKey } from '../../../../scripts/capacity-identity-epoch/exclusion';
...
function runBridge(command: 'acquire' | 'assert' | 'renew' | 'release', descriptor: unknown): Promise<BridgeResult> {
    const script = resolve(fileURLToPath(new URL('../../../../scripts/check-capacity-identity-epoch-exclusion.ts', import.meta.url)));
