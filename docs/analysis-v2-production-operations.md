...
1. checkout은 서버 카탈로그 및 preflight snapshot에서만 Basic/Standard를 만들며, 현재 가격 버전은 `earlybird-2026-08-v5`다. 기존 v1/v2/v3/v4 payment lineage는 새 checkout으로 바꾸지 않고 immutable snapshot을 보존한다([`20260812122517_update_earlybird_pricing_v5.sql`](../supabase/migrations/20260812122517_update_earlybird_pricing_v5.sql), [`app/api/earlybird/checkout/route.ts`](../app/api/earlybird/checkout/route.ts)).
