...
score audit 보존 정리의 FK invariant는 `analysis_v2_score_audit_runs` child를 만들 때 `analysis_v2_result_summaries` parent가 반드시 존재해야 한다는 것이다. working-set 정리가 parent summary를 먼저 없앤 뒤에도 request에 매달린 queued intent가 TTL까지 남을 수 있으므로, hard-TTL drain은 intent를 잠그고 summary 존재 여부를 확인한 뒤 summary가 있으면 `partial/SOURCE_EVIDENCE_EXPIRED` run을 만들고, 없으면 run을 만들지 않고 intent와 final checkpoint만 release/purge한다. 이 순서는 기존 audit 기록과 retention 경계를 보존하며, 광범위한 `CASCADE` 삭제를 추가하지 않는다([`20260901192353_fix_analysis_v2_score_audit_expiry_orphans.sql`](../supabase/migrations/20260901192353_fix_analysis_v2_score_audit_expiry_orphans.sql)).
