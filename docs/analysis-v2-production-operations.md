...
- canonical worker는 admission, worker, recovery, Cloud Tasks, preflight, automatic fulfillment을 켠 production 경로다. Cloud Tasks의 target은 `/api/analysis/v2/worker`만 허용하며 OIDC audience도 같은 origin이어야 한다([`lib/services/analysis/v2-tasks.ts`](../lib/services/analysis/v2-tasks.ts)).
- secondary Apify 계정이 선택되어 있다. 검증용 `analysis-worker-secondary-e2e`는 보존하되 worker/recovery/tasks/preflight를 모두 `false`로 유지한다. 그것은 production queue의 대체 대상이 아니다.
...
1. checkout은 서버 카탈로그 및 preflight snapshot에서만 Basic/Standard를 만들며, 가격 버전은 `earlybird-2026-07-v2`다. 불명확한 기존 payment lineage는 새 checkout으로 바꾸지 않고 safe classification으로 돌려준다([`20260728130000_classify_earlybird_checkout_lineage.sql`](../supabase/migrations/20260728130000_classify_earlybird_checkout_lineage.sql), [`app/api/earlybird/checkout/route.ts`](../app/api/earlybird/checkout/route.ts)).
...
Cloud Tasks의 canonical queue는 canonical `analysis-worker`로만 dispatch한다. job identity는 request, job key, generation, reservation token으로 fence되고 동일 generation Task 이름은 재사용된다([`lib/services/analysis/v2-tasks.ts`](../lib/services/analysis/v2-tasks.ts), [`lib/services/analysis/v2-job-store.ts`](../lib/services/analysis/v2-job-store.ts)).
...
- inbound 후보가 하나라도 있으면 high pool은 inbound만이다. 하나도 없으면 eligible 전체가 high pool이다.
- eligible이 3명 미만이면 natural 결과를 유지한다. 3명이면 high 최소 1명이다.
- eligible이 4명 이상이면 high 최소 2명이다.
- eligible이 5명 이상이고 high pool이 3명 이상이며 세 번째 natural public score가 4.2 이상일 때만 high 3명을 floor로 둘 수 있다.
- high는 최대 3명, caution은 최소 2명·최대 10명이다. 후보→대상과 대상→후보 tag component는 모두 score에 보존한다.
...
- worker rollback은 canonical deployment의 admission/worker/recovery와 `EARLYBIRD_AUTOMATIC_FULFILLMENT_ENABLED`을 필요한 범위에서 끄고, 이미 durable admission 상태인 작업은 recovery로 drain한다. 결제 상태를 rollback 수단으로 바꾸지 않는다.
