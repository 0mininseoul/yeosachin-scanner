...
- 결제·주문·Groble webhook 경로에서는 서버에서 Amplitude 이벤트를 전송하지 않는다. 예외는 분석 파이프라인 terminal lifecycle(`analysis_started`, `analysis_completed`, `analysis_failed`)과 인증된 공유 관측 endpoint/Kakao Share webhook의 bounded 공유 사건이다. 둘 다 원장의 request UUID와 닫힌 상태만 best-effort로 보내며 전송 실패가 제품 흐름을 실패시키지 않는다.
...
`payment_confirmed_viewed`의 의미는 이름 그대로 결제 확인 화면 조회다. `paid`·`analysis_in_progress`·`completed` 주문의 화면을 본 사실을 기록할 뿐 결제 승인이나 매출을 뜻하지 않으며, 이름은 기존 대시보드 호환을 위해 유지한다. 실매출은 Supabase에서 `payment.completed` webhook event의 `disposition=accepted`, 연결된 order, 양수 금액·paid_at이 모두 일치하고 `traffic_class=external`인 lineage만 확정한다. `payment_id + paid_at + 양수 금액`만으로는 합성 E2E 주문을 걸러낼 수 없다.
...
- 금지 속성 검사: `email`, `phone`, `name`, `instagram`, `username`, `profile`, `bio`, `comment`, `caption`, `image`, `media`, `url`, `token`, `cookie`, `signature`, `body`, `response` 이름이나 실제 민감 값이 event·user properties에 없는지 검사한다. Replay UI에서는 결과·공유 결과·preview의 카드·일반 텍스트·이미지는 보이고 로그인·결제·target/freeform 입력, 연락처, query/token, raw 오류는 보호되는지 확인한다.
- 원장 대조: `payment_confirmed_viewed`는 같은 기간 화면 조회와만 비교하고 매출로 쓰지 않는다. 매출은 accepted webhook/order와 external traffic class로 대조한다. 서버 `analysis_completed` 수는 `analysis_v2` terminal 성공 요청 수와 비교하고, 기존 클라이언트 이벤트와의 차이를 누락량으로 별도 기록한다.
