...
- 런타임과 Groble 얼리버드 채널의 가격 정본은 `pricingVersion=earlybird-2026-08-v3`다. 가격은 결제 검증 snapshot이므로 코드·DB·Groble 상품 화면을 함께 대조한다.

## Groble 얼리버드 가격

Basic과 Standard는 모두 72% 할인으로 표시한다. 표시 할인율은 판매 운영값이며, 기준가와 결제액의 산술 할인율로 다시 계산하지 않는다. 정식 자동 분석 런칭 뒤에도 할인가 판매를 유지할 수 있으므로 기준가는 서버 표시 카탈로그에 두고 실제 결제액은 preflight와 주문의 immutable snapshot으로 보존한다.

| 플랜 | 기준가 | 얼리버드 결제액 | 표시 할인율 | 제공 방식 |
|---|---:|---:|---:|---|
| Basic | 3,990원 | 990원 | 72% | 자동 분석 공개 입장 |
| Standard | 7,990원 | 1,990원 | 72% | 자동 분석 공개 입장 |
| Plus | - | 대기 신청 | - | 대기 신청 |
