...
결제 전 대상 프로필 사전 점검은 로그인 없는 자체 summary 수집을 먼저 시도한다. 대상 없음이 자체 수집에서 명시적으로 확인되면 유료 fallback을 실행하지 않는다. 분류된 자체 provider 실패일 때만 preflight당 단 한 번의 Apify profile-summary fallback을 허용한다. 실행 전 원장을 예약하고 `maxTotalChargeUsd=$0.0026`을 고정하며, retry는 저장된 같은 run ID만 재개한다. Instagram 로그인 쿠키나 세션은 전달하지 않는다.
...
- `P`: 결제 전 대상 프로필 preflight에서 실행한 Apify summary fallback 수. preflight당 `P ∈ {0,1}`
...
Actor별 실제 단가를 각각 `u_preflight`, `u_rel`, `u_profile`, `u_liker`, `u_comment`라 하면 외부 수집 비용은 다음과 같다. `u_preflight`는 30초 후 인증된 run 재조회로 확정한 `usageTotalUsd`이며 `0 <= u_preflight <= $0.0026`이다.

```text
C_provider = P * u_preflight
           + (F + G) * u_rel
           + B * u_profile
           + LT * u_liker
           + CT * u_comment
...
`C_total`은 소비된 preflight의 `P * u_preflight`를 해당 분석에 귀속한다. 사전 점검 후 결제/분석으로 전환되지 않고 만료·이탈한 preflight도 fallback이 실행됐다면 최대 `$0.0026`의 획득 원가가 이미 발생한다. 기간 총원가와 판매가 산정은 성공 분석만 모수로 보지 말고, 모든 생성 preflight의 `P * u_preflight`를 합산한 뒤 유료 전환 건에 배부해야 한다. Apify fallback이 대상 없음을 확인한 경우에도 유저 결제는 없지만 이 운영 원가는 발생할 수 있다.
...
2. 모든 생성 preflight의 `P`, Apify summary run id·상한·실제액과 유료 분석 전환 여부를 기록한다. 관계 선언/반환/고유 수, 정확한 `B`, `LT`, `CT`, 각 `R_i`, 분석 Actor run id와 실제 청구액도 함께 기록한다.
...
- `analysis_preflight_provider_runs`: 결제 전 단일 Apify target-profile summary fallback의 예약, run id, credential slot, `$0.0026` 상한, terminal 상태, provider `finishedAt`이 최소 30초 지난 후의 실제 사용액. 만료·이탈 preflight도 정산 전에는 원장을 삭제하지 않는다.
