...
- 얼리버드 현재 가격은 Basic **1,990원**, Standard **2,990원**이다. 가격 snapshot은 `earlybird-2026-08-v4`이며, 가격 유지가 complete cost의 증거는 아니다.
...
- 런타임과 Groble 얼리버드 채널의 가격 정본은 `pricingVersion=earlybird-2026-08-v4`다. 가격은 결제 검증 snapshot이므로 코드·DB·Groble 상품 화면을 함께 대조한다.

## Groble 얼리버드 가격

Basic은 50%, Standard는 62% 할인으로 표시한다. 표시 할인율은 기준가와 결제액의 산술 할인율을 과장하지 않도록 정수 단위로 내린 값이다. 정식 자동 분석 런칭 뒤에도 할인가 판매를 유지할 수 있으므로 기준가는 서버 표시 카탈로그에 두고 실제 결제액은 preflight와 주문의 immutable snapshot으로 보존한다.

| 플랜 | 기준가 | 얼리버드 결제액 | 표시 할인율 | 제공 방식 |
|---|---:|---:|---:|---|
| Basic | 3,990원 | 1,990원 | 50% | 자동 분석 공개 입장 |
| Standard | 7,990원 | 2,990원 | 62% | 자동 분석 공개 입장 |
| Plus | - | 대기 신청 | - | 대기 신청 |

Basic과 Standard는 각각 독립적으로 선착순 10건씩만 받는다. Plus는 checkout 없이 대기 신청만 유지한다.

Groble 결제 건당 수수료 가정은 실제 운영 조건인 **8.69%**다. 이 비율은 결제 webhook 금액 검증에 사용하지 않고 마진 계획에만 사용한다. 단순 산술상 Basic은 `1,990 × (1 - 0.0869) = 1,817.07원`, Standard는 `2,990 × (1 - 0.0869) = 2,730.17원`이다. 원 단위 반올림 계획 순수입은 각각 **1,817원**, **2,730원**이다. 실제 정산액과 세금 처리는 Groble 정산 명세를 정본으로 대사한다.
