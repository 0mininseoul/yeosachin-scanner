...
기준일: 2026-07-25. 이 문서는 현재 V2 코드의 **과금 단위와 측정 방법**을 정리한다. 2026-07-24 Plus 통제 E2E 1건은 완료됐지만 비용 완전성이 부족하고 Basic/Standard 통제 표본과 p50/p95가 없으므로 건당 완전 원가는 확정하지 않는다. Actor 콘솔 가격, 과거 V1 canary, 환경변수의 예산 상한을 V2 실측 원가로 대체해서는 안 된다.
...
|---|---:|---:|---:|---|
| Basic | 13,900원 | 6,900원 | 50% | 결제 후 24시간 이내 수동 전달 |
| Standard | 19,900원 | 9,900원 | 50% | 결제 후 24시간 이내 수동 전달 |
| Plus | - | 대기 신청 | - | 대기 신청 |
...
- V2 cost reconciliation, 유료 성공 E2E, deployment-wide Gemini lease, Basic/Standard 통제 표본이 완료되기 전 자동 분석 dispatch나 public analysis admission을 열지 않는다. 현재 Groble checkout/webhook은 24시간 이내 수동 전달 계약이며 결제 확정이 분석 작업을 만들지 않는다.
