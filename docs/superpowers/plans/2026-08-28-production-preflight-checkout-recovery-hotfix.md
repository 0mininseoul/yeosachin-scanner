...
- [ ] Move the legacy missing-hash branch before B-lite activation and emit a
...
### Task 6: Add the fail-closed administrator blocker cleanup

**Files:**
- Create a one-shot migration with `supabase migration new`.
- Add migration contract assertions following repository conventions.
...
- [ ] Require the exact target, Standard plan, pending status, absent payment
  evidence, absent fulfillment/result evidence, and exact expected row count.
- [ ] Remove or terminalize only the blocking test order using the least
  destructive operation compatible with current foreign keys and audit rules.
- [ ] Fail the migration if any precondition differs.
- [ ] Prove the administrator account and every external-user order are
  untouched.
- [ ] Do not apply this migration remotely from the worker.
...
- [ ] Review the diff for raw Groble URLs in client responses, secrets, broad RLS
  ...
  artifacts, and unrelated refactors.
...
- [ ] In an isolated Supabase workdir, allowlist only the reviewed migrations and
  run dry-run.
- [ ] Apply only those migrations, then verify remote history before any retry.
- [ ] Deploy only to the existing `yeosachin_scanner` Vercel project.
