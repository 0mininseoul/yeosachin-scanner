...
**Tech Stack:** PostgreSQL/Supabase, Next.js/TypeScript, Vitest, PGlite or disposable PostgreSQL, `npx supabase@2.102.0`, and the existing operations/evidence verifiers.
...
closed quiescence proof. Current classification is 22 `retain`, 140
`consolidate`, 0 `retire`, and 15 `blocked`.
...
- Use `npx supabase@2.102.0`; use the linked authenticated project and an
  isolated temporary CLI work directory when the canonical worktree has
  application environment-file parsing issues. Never print or persist tokens,
  database passwords, cookies, UUIDs, raw provider payloads, or user/device
  identifiers.
- Do not run `supabase db push --include-all` in a dirty or mixed worktree. A
  future apply must use a clean isolated worktree and a literal migration
  allowlist after a dry run.
...
- `waves.wave1AnalysisCanonicalization.sourceAllowlist`: 102 analysis source
  tables; destination families are
  `account_lifecycle`, `analysis_artifacts`, `analysis_audit_bundles`,
  `analysis_cache`, `analysis_costs`, `analysis_events`, `analysis_jobs`,
  `analysis_preflights`, `analysis_provider_runs`, `analysis_requests`,
  `analysis_results`, `maintenance_jobs`, and `system_configuration`.
- `waves.wave2CommerceOperationsCanonicalization.sourceAllowlist`: 38
  commerce/operations source tables; destination families are
  `account_lifecycle`, `fulfillment_jobs`, `maintenance_jobs`,
  `notification_outbox`, `payment_events`, `system_configuration`, and
  `system_leases`.
...
The 15 blocked names requiring new evidence are:
...
owner catalog and preservation contract exist. The order-audit family remains
...
- [ ] Create an isolated temporary directory containing only the linked
  Supabase CLI metadata needed by `npx supabase@2.102.0`. Run only read-only
  `db query --linked --output json` catalog queries and `migration list --linked`.
  ...
  locks, and sanitized static caller counts.
- [ ] Regenerate the inventory and assert `177`, `22`, `155`, `0`, the exact
  ...
  `22/140/0/15`. Reject duplicate or missing names and reject any sensitive
...
- [ ] Run `git diff --check` and the report/schema tests before any future
  implementation wave. This wave is complete only when the sanitized report
  is committed and the coordinator records the exact commit SHA.
...
## Wave 1: analysis canonicalization, additive only

**Purpose:** Converge the 102 exact analysis source tables into the listed
canonical families without deleting source data.

- [ ] Use the existing analysis canonicalization plan and its named files for
  the six canonical families: jobs, events, artifacts, costs, cache, and audit
  bundles. Reuse existing aggregate names and keep
  `analysis_pipeline_jobs`, DAG/progress/evidence/cost/cache, and permanent
  audit sources authoritative during rollback.
- [ ] Add server-only, typed dual-write adapters behind family flags, all
  defaulting to `false`. If a transaction cannot dual-write, enqueue a bounded
  retry in `maintenance_jobs`; do not claim parity from a failed write.
...
- [ ] Require zero unresolved parity mismatches, bounded retry drain, no
  forbidden payload/token columns, RLS/ACL checks, and a tested reader rollback
  before enabling any family flag. Enabling a flag is outside this read-only
  task and requires a separate owner approval.
...
`--include-all`; run `npx supabase@2.102.0 db push --dry-run --linked` from an
isolated clean worktree and verify that the output contains only that literal
set. After an approved apply, verify migration history exactly once, canonical
...
**Wave 1 rollback:** set all analysis canonical read flags false, drain or
  fence canonical retry jobs, return reads to the source tables, and retain
  canonical rows for forensic comparison. If a migration itself must be
  reversed, use the migration's explicit reversible path from a clean isolated
  worktree after verifying remote history; never repeat a hung push.

## Wave 2: commerce and operations canonicalization, additive only

**Purpose:** Converge the 38 exact commerce/operations source tables while
preserving payment, fulfillment, notification, account, lease, configuration,
and maintenance evidence.

- [ ] Use the existing commerce/operations canonicalization plan and its named
  files for `payment_events`, `fulfillment_jobs`, `notification_outbox`,
  `account_lifecycle`, `system_configuration`, `system_leases`, and
  `maintenance_jobs`.
...
**Wave 2 dry-run and post-apply gate:** use a literal list containing only the
generated commerce/operations migration version(s) in
`npx supabase@2.102.0 db push --dry-run --linked`; do not use
...
The current approved retirement set is exactly `[]`. The current 15 blocked
...
- [ ] Prove no incoming or outgoing FK, dependent view, stored routine
  ...
  external writer. A stored-routine mention or a dependency edge keeps the
  table blocked even if it is empty.
...
are approved, run `npx supabase@2.102.0 db push --dry-run --linked` in a clean
isolated worktree and assert that only the exact migration is selected. Do not
run an apply in this plan. If a future apply hangs, inspect remote migration
history and catalog state first; do not repeat the command blindly.
...
retirements have independent archive/restore proof, and all 15 current blocked
...
- [ ] Run bounded smoke tests for analysis request/result, payment event
  idempotency, fulfillment lease fencing, notification dedupe, account
  lifecycle, maintenance retry, archive/restore, and the read-only catalog
  verifier. Keep activation and real canary state untouched.
...
Run these from the clean implementation worktree after each code/migration
wave, never against production during this read-only task:

```bash
npx vitest run \
  lib/services/operations/supabase-22-evidence.test.ts \
  lib/services/operations/supabase-22-catalog-pglite.test.ts \
  lib/services/operations/supabase-22-evidence-traffic.test.ts \
  scripts/verify-supabase-22-catalog.test.ts \
  scripts/verify-supabase-22-retirement-inventory.test.ts \
  scripts/verify-supabase-22-archive-restore.test.ts \
  scripts/verify-supabase-22-comment-interaction-retirement.test.ts \
  scripts/verify-supabase-22-legacy-earlybird-retirement.test.ts
npx vitest run \
  lib/services/analysis/canonical-analysis-store.test.ts \
  lib/services/analysis/canonical-analysis-pglite.test.ts \
  lib/services/analysis/canonical-analysis-read.test.ts \
  scripts/backfill-analysis-canonical.test.ts \
  lib/services/commerce/canonical-commerce-store.test.ts \
  lib/services/commerce/canonical-commerce-pglite.test.ts \
  lib/services/operations/canonical-operations-store.test.ts \
  scripts/backfill-commerce-operations-canonical.test.ts \
  scripts/backfill-account-deletion-canonical.test.ts
...
if (d.liveState.publicBasePartitionedTableCount !== 177
  ...
  || d.classificationSummary.consolidate !== 140
  || d.classificationSummary.retire !== 0
  || d.classificationSummary.blocked !== 15
...
intended diff, `git diff --check` passes, focused tests and the report contract
