...
**상태:** Phase A 계약과 Phase B 보호된 사전 점검 구현·검증 완료. Phase C 작업 기반이 다음 단계
...
- 파트너 근거는 100점 합산과 별도로 처리한다. 또래로 보이는 남성과 둘이 찍은 사진 한 장은 약한 감점 근거다. 연예인·공인 또는 명확한 연상 가족이면 예외다. 반복되고 강한 파트너 근거가 있으면 공개 점수를 3.4로 제한해 정상군에 두되 관측 사실은 내부에 보존한다.
...
고정 순차 임계값이 아니라 DAG work-unit으로 단조 증가 진행률을 발행한다. 마스킹된 현재 프로필, 병렬 트랙 상태, 퍼센트, ETA, provisional/correction 이벤트를 제공한다. 숫자 퍼센트가 같아도 terminal 상태, 현재 프로필, 이벤트 sequence 등 공개 snapshot의 의미가 바뀌면 revision을 올리고 이벤트 append 시 sequence 연속성을 강제한다. snapshot을 먼저 hydrate하고 Realtime 구독 후 sequence gap을 보정하며 5초 polling을 최종 fallback으로 둔다. 결과에는 범위 커버리지, 실제 1.0~10.0 점수, 별도 대표 섹션, 한 줄 개요, 고위험 없음 상태, pagination, 긴 목록 virtualization을 추가한다. reduced-motion, 모바일, 이미지 실패, reconnect, 브라우저 복귀, terminal failure를 처리한다.
...
- 조정 전 보류: 약한 남성 동반자 감점과 강한 파트너 신뢰 기준은 라벨 fixture로 결정하되, 강한 근거의 정상군 상한 자체는 고정한다.

### Phase A를 막지 않는 보류 결정

최종 Basic/Standard/Plus 원화 가격·결제 provider, 파트너 감점·신뢰 기준, Stage 2가 Flash-Lite medium인지 Gemini 3 Flash medium인지, Cloud Run 리전/CPU/메모리/동시성/Gemini quota, Standard/Plus 출시 여부는 E2E·A/B·부하·비용 게이트 뒤에 결정한다.
