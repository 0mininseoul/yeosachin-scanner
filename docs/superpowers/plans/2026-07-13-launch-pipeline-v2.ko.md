...
1. 결제 전에 자체 수집 우선 대상 계정 사전 점검을 실행한다. 분류된 자체 provider 실패일 때만 상한이 고정된 Apify profile-summary fallback 1회를 허용한다.
...
Vercel UI/API
  |
  +-- 사전 점검 API -- 자체 summary 우선, 제한 fallback -- Supabase preflight
  |
  +-- 이용권/결제 경계
             |
             v
...
`POST /api/analysis/preflight`는 요청을 저장하고 대상 프로필 작업을 큐에 넣은 뒤 즉시 `pending`을 반환한다. `GET /api/analysis/preflight/:id`는 `pending`, `ready`, `blocked` 상태와 대상 username, 이름, bio, 아바타, 팔로워/팔로잉 수, 비공개 여부, 필요한 플랜, 플랜 견적, 가격 버전을 반환한다.
...
2. `analysis_preflight_provider_runs`: 단일 Apify 대상 profile-summary fallback의 PII 없는 예약·비용 원장. run ID, credential slot, 고정 상한, terminal 상태, provider 완료 시각, 지연 실제액을 보존한다. 기간 비용 정본은 장기 확정 이벤트와 현재 미정산 최대 노출을 함께 반환하는 aggregate RPC이며 원장 직접 합계가 아니다. 대상 identity는 인증·이미지·provider 키와 분리된 안정적인 `ANALYSIS_V2_PREFLIGHT_IDENTITY_HMAC_SECRET`으로 domain-separated HMAC 처리한다.
3. `analysis_pipeline_jobs`: 요청, job key, track, kind, batch, 상태, 입력 hash, lease fencing token, 시도 횟수, 시간, 에러 코드
4. `analysis_profile_fetch_results`: username, source(`cache/selfhosted/apify`), 상태, 제한된 프로필/미디어 snapshot, 실패 원인, 수집 시각
5. `analysis_candidate_ai_results`: 단계(`triage/features/partner_safety`), 모델/thinking/prompt/schema 버전, 입력 hash, 엄격한 결과
6. `analysis_target_interactors`: 성별 확정 전 저장하는 제한된 대상 liker/댓글 staging. service-role만 읽고 종료 시 삭제
7. `analysis_progress_state`, `analysis_progress_events`: 소유자가 읽을 수 있는 정리된 상태/이벤트, 단조 증가 revision/sequence
8. `analysis_v2_active_profile_heartbeats`: service 전용 마스킹 시작 heartbeat. 살아 있는 프로필 작업별 1행을 두고 소유자 progress 조회에는 가장 최근 시작 행 하나만 투영
...
migration/RLS/만료/멱등성, 비동기 preflight API, 자체 Cloud Run 작업, 제외 route, 서명 테스트 이용권을 추가한다. 초기 Phase B acceptance는 유료 provider 0건이었다. 현재 운영 결정은 이 항목만 대체한다. 분류된 자체 provider 실패에서만 Instagram 로그인 쿠키 없는 Apify profile-summary fallback 1회를 허용한다. 자체 성공/명시적 대상 없음은 유료 provider 0건을 강제하고, 허용된 실패는 시작 전 예약·`$0.0026` 상한·같은 run 재개·30초 후 정산을 검증한다. Gemini·관계·상호작용은 항상 0건이다.
...
- cache miss 자체 성공 사전 점검 p95 < 5초
- Apify fallback을 시작하거나 재개하는 worker 시도는 provider hard deadline 105초. fallback 지연은 자체 성공 p95와 분리 측정
- 첫 유용한 확정 진행률 < 60초
...
| 실패 | 사용자 상태 | 구제 |
|---|---|---|
| 대상 없음/비공개 | 사전 점검 차단 | 유저 결제 없음. fallback이 실행됐다면 운영 원가는 최대 `$0.0026`, username 수정 |
...
2. preflight/exclusion schema·RLS·API·자체 무비용 acceptance·제한된 단일 fallback 비용/멱등성 테스트
