...
- 외부 결과는 남성·여성·미상만 제공한다. 실패 원인과 resolver 상태는
  서비스 전용 지표에만 남긴다.
- 관측된 미상 비율 30% 이하는 배포 승인 기준이지 결과 생성 규칙이 아니다.
...
경우까지 모두 미상으로 남긴다. 최근 완료 결과에서 공개 계정 미상 비율이 30%를
...
미상 30% 초과 시 E2E 결과를 수정하거나 계정을 강제로 남성·여성으로 바꾸지 않는다.
...
1. 고급 resolver는 불확실 triage에만 기회주의적으로 실행하고 고신뢰 여성 triage에는
   기본 실행하지 않는다.
2. 미상 30%는 rollout gate로만 사용하며 결과 강제 조정 규칙으로 사용하지 않는다.
